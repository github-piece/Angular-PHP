<?php
	...
	function getArticleList($param, $conn){
		...
		$sql = "SELECT tbl_article.*, tbl_user.* FROM tbl_article left join tbl_user on tbl_article.article_uid = tbl_user.u_id".$where." order by tbl_article.article_createdate DESC";
	...
	function addArticle($param, $conn){
		...
		$sql = "INSERT INTO tbl_article (section1, section2, headline, account_type, article_uid, article_uparentid, article_createdate, imgurl1, imgurl2) VALUES('".$param['section1']."', '".$param['section2']."', '".$param['headline']."', '".$param['u_accounttype']."', '".$param['u_id']."', '".$param['u_parentid']."', '". $date."', '".$filename1."', '".$filename2."')";
		$result = $conn->query($sql);
		getArticleList($param, $conn);
	}
	
	function updateArticle($param, $conn){
		$sql = "UPDATE tbl_article SET section1 = '".$param['section1']."', section2 = '".$param['section2']."', headline = '".$param['headline']."'";
