         ...
         foreach($key_scoring_info as $key_info){
             ...
             foreach($scoringData as $key_data=>$value){
                 foreach($value as $row_item){
                     $scoringDataList[$key_info['description']] += floatval($row_item[strtolower($key_info['description'])]);
                 }
             }
         }
            ...
            foreach($goals as $goal){
                foreach ($goal_names as $goal_name){
                    if($goal['goal_name']===$goal_name){
                        $unSdg_item['goal_number'][] = $goal['goal_number'];
                        $unSdg_item['goal_description'][] = $goal['short_description'];
                        $unSdg_item['path'][] = $goal['path'];
                    }
                }
            }
            $unSdg[] = $unSdg_item;
